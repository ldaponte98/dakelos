...
@section('content')
    ...
    <div class="row">
        <div class="col-sm-8">
            <div class="d-flex">
                <input type="text" placeholder="Consulta productos aqui..." onkeyup="BuscarProductos(this.value)"
                    class="form-control search mb-2" id="filtro-productos">
                <button class="btn btn-secundary btn-erase" onclick="$('#filtro-productos').val(''); BuscarProductos('')"><i
                        ...
                        @foreach ($categorias as $item)
                            <li class="nav-item">
                                <a class="nav-link {{ $cont == 0 ? 'active' : '' }}"
                                    id="nav-category-{{ $item->id_categoria }}-tab" data-toggle="pill"
                                    href="#nav-category-{{ $item->id_categoria }}" role="tab"
                    ...
                    <div class="tab-content" id="pills-tabContent">
                        ...
                        @foreach ($categorias as $item)
                            <div class="tab-pane fade {{ $cont == 0 ? 'show active' : '' }}"
                                id="nav-category-{{ $item->id_categoria }}" role="tabpanel"
                                                    ...
                                                    <div class="card-body">
                                                        <div class="mx-auto d-block">
                                                            <img width="100" height="100"
                                                                ...
                                                                src="{{ $producto->get_imagen() }}" alt="Producto">
                                                            <h5 class="text-sm-center mt-2 mb-1">{{ $producto->nombre }}
                                                            </h5>
                                                        </div>
                                                    </div>
                                                </div>
            ...
            <div class="row">
                <div class="col-sm-6">
                    <div class="form-group">
                        <select id="select-canal" onchange="ValidarCanal(this.value)" data-placeholder="Selecciona un canal"
                ...
                <div class="col-sm-6" id="div-mesas" style="display: none;">
                    <div class="form-group">
                        <select id="select-mesa" onchange="AsignarMesa(this.value)" data-placeholder="Seleccione la mesa"
                            class="form-control select2">
                            <option value="" label="default"></option>
                ...
                <div class="col-sm-6" id="div-direccion" style="display: none;">
                    <div class="form-group">
                        <input placeholder="Dirección" type="text" id="factura-direccion-domicilio" placeholder="0"
                            class="form-control">
                    </div>
                </div>
            </div>
        ...
        <div class="modal fade" id="modal-anulacion" tabindex="-1" role="dialog" aria-labelledby="smallmodalLabel"
            aria-hidden="true">
    ...
    <script>
        ...
        function ActualizarVistaPedido() {
            ...
            let domicilio = $("#factura-domicilio").val()
            if ($.isNumeric(domicilio) && this.factura.id_dominio_canal != {{ App\Dominio::get('Mesa') }}) total +=
                parseFloat(domicilio)
        ...
        function BuscarProductos(caracteres) {
            if (caracteres.trim() != "" && caracteres.length >= 3) {
                let filtros = this.productos.filter(item => item.nombre.toUpperCase().includes(caracteres.toUpperCase()) &&
                    item.tipo == 36)
                var resultados = ""
                if (filtros.length > 0) {
                    let cont = 0
        ...
        function ValidarCanal(id_dominio_canal) {
            ...
            if (id_dominio_canal != {{ App\Dominio::get('Mesa') }} && id_dominio_canal !=
                {{ App\Dominio::get('Domicilio') }}) {
                $("#div-mesas").fadeOut()
                $("#div-domicilio").removeClass('hide')
                $("#div-direccion").fadeIn()
                this.factura.domicilio = 0
