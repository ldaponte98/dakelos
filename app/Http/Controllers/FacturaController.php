...
class FacturaController extends Controller
{
    public function crear(Request $request)
    {
    	...
    	$accion = "Facturación";
    	$tipo = $post->tipo;
    	if($tipo == 17) $accion = "Cotización";
        if($post->id_tercero) $tercero = Tercero::find($post->id_tercero);
    ...
    public function finalizar_factura(Request $request)
    {
        ...
        if($post){
            ...
            if($resolucion){
                $factura = new Factura;
                if($post->tipo_factura == 16) $factura->numero = $resolucion->prefijo_factura."-".($resolucion->consecutivo_factura + 1);
                if($post->tipo_factura == 17) $factura->numero = $resolucion->prefijo_cotizacion."-".($resolucion->consecutivo_cotizacion + 1);
                ...
                if($factura->save()){
                    ...
                    if($post->tipo_factura == 16) $resolucion->consecutivo_factura += 1;
                    if($post->tipo_factura == 17) $resolucion->consecutivo_cotizacion += 1;
                    $resolucion->save();
